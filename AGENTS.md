...
- [ ] `app/src/main/java/com/example/ui/AboutScreen.kt` — `versionLogs` 内置更新记录：在列表**顶部**插入新版本条目（版本号、日期、简洁更新清单）

**APP 内更新记录撰写规则（面向普通用户，与 GitHub 更新记录刻意保持不同）：**

- 更新记录为应用内置，**不从 GitHub 实时拉取**（避免中国大陆网络不可达），随版本迭代时更新
- 过滤或改写不适合普通用户看到的信息：
  - 包名变更等技术信息 → 直接过滤
  - 新增某 SDK：若为用户带来新功能 → 写「新增某功能」；若仅对开发者有用、用户无感知 → 直接过滤
- 若某次迭代内容均被过滤，则默认记录为「一些细节优化/已知问题修复」
